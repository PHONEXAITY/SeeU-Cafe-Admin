...
export const EditProductModal = ({
  ...
  // Only render if we have a product to edit
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 3. Update the product based on its type
      if (productToEdit.type === "food") {
        productData.price = parseFloat(formData.price) || 0;
        await productService.updateFoodProduct(productToEdit.id, productData);
        ...
        await productService.updateBeverageProduct(productToEdit.id, productData);
      ...
      await queryClient.invalidateQueries(
        productToEdit.type === "food" ? ["foodProducts"] : ["beverageProducts"]
      );
  
    } catch (error) {
      console.error("Error updating product:", error);
      const errorMessage = error.response?.data?.message || error.message;
      toast.error(`ແກ້ໄຂສິນຄ້າລົ້ມເຫລວ: ${errorMessage}`);
    }
  };
      ...
      >
        <DialogHeader className="border-b pb-3">
          ...
          <DialogDescription className="text-gray-600">
            ແກ້ໄຂຂໍ້ມູນລາຍລະອຽດຂອງສິນຄ້າ "{formData.name}"
          </DialogDescription>
        </DialogHeader>
