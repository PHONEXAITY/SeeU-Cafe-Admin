    ...
    try {
      ...
      if (
        ...
        if (formData.restrictions.newCustomersOnly) {
          restrictionText.push("ສຳລັບລູກຄ້າໃໝ່ເທົ່ານັ້ນ");
        }
        if (formData.restrictions.membersOnly) {
          restrictionText.push("ສຳລັບສະມາຊິກເທົ່ານັ້ນ");
        }
        if (formData.restrictions.oneTimeUse) {
          restrictionText.push("ໃຊ້ໄດ້ພຽງຄັ້ງດຽວຕໍ່ຄົນ");
        }

        const restrictionString = restrictionText.join(", ");

        if (restrictionString) {
          promotionData.description = promotionData.description
            ? `${promotionData.description} (${restrictionString})`
            : restrictionString;
        }
      }
      ...
      toast({
        title: "ສຳເລັດ",
        description: "ສ້າງໂປຣໂມຊັ່ນສຳເລັດແລ້ວ",
      });

      router.push("/promotions/active");
    } catch (err) {
      console.error("Error creating promotion:", err);

      let errorMessage = "ບໍ່ສາມາດສ້າງ promotion ໄດ້";
      ...
      if (err.response?.data?.message) {
        ...
        if (
          errorMessage.includes("Promotion with code") &&
          errorMessage.includes("already exists")
        ) {
          errorMessage = `ລະຫັດໂປຣໂມຊັ່ນ "${formData.code}" ຖືກໃຊ້ແລ້ວ, ກະລຸນາລອງລະຫັດອື່ນ`;
        } else if (errorMessage.includes("End date must be after start date")) {
          errorMessage = "ວັນທີສິ້ນສຸດຕ້ອງຢູ່ຫຼັງຈາກວັນທີເລີ່ມຕົ້ນ";
        }
      }
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <Badge variant="outline" className="rounded-full">
              <Clock className="w-3 h-3 mr-1" /> ໃໝ່
            </Badge>
          </div>
          ...
          <p className="text-muted-foreground mt-1">
            ສ້າງແລະຕັ້ງຄ່າ promotion ເພື່ອດຶງດູດລູກຄ້າແລະເພີ່ມຍອດຂາຍ
          </p>
        </div>
                    ...
                    <div className="space-y-2 mb-3">
                      ...
                      <div className="flex items-center justify-between text-sm p-2 bg-muted/20 rounded-md">
                        <div className="flex items-center gap-2">
                          ...
                          <span>
                            {formData.start_date &&
                            isValid(new Date(formData.start_date))
                              ? format(
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span>
                            {formData.end_date &&
                            isValid(new Date(formData.end_date))
                              ? format(
            ...
            <CardContent className="pt-0">
              ...
              <div className="mt-4">
                ...
                >
                  ...
                  ຮຽນຮູ້ເພີ່ມເຕີມກ່ຽວກັບໂປຣໂມຊັ່ນ
                </Link>
              </div>
            </CardContent>
          </Card>
